...
{
    // build url
#ifdef TELEGRAM_API_BASE
    QString urlStr = QString(TELEGRAM_API_BASE).arg(this->apiKey, method);
    QUrl url(urlStr);
#else
    // Default behavior
...
// Only set port if specifically requested (e.g. for Local API)
#ifdef TELEGRAM_API_PORT
    url.setPort(TELEGRAM_API_PORT);
#endif
    url.setQuery(params);
...
QHttpMultiPart* TelegramBot::handleFile(QString fieldName, QVariant file, QUrlQuery &params, QHttpMultiPart* multiPart)
{
    ...
    // handle url
     else if(file.type() == QVariant::String || file.type() == QVariant::Url) {
//        QUrl url = QUrl::fromUserInput(file.toString());
        QUrl url = (file.type() == QVariant::Url) ? file.toUrl() : QUrl::fromUserInput(file.toString());

        // upload the local file to telegram
        if(url.isLocalFile() || url.isRelative()) {
            QString localPath = url.isLocalFile() ? url.toLocalFile() : url.toString();
            QFile fFile(localPath);
